...
def _extract_slice(image: sitk.Image, plane: str, idx: int) -> sitk.Image:
    """Extract a 2D slice using SimpleITK Extract to preserve orientation."""
...
def to_uint8_mri(arr2d: np.ndarray) -> np.ndarray:
    nonzero = arr2d[arr2d != 0]
    if nonzero.size > 0:
        p1 = np.percentile(nonzero, 1)
        p99 = np.percentile(nonzero, 99)
    else:
        p1 = np.percentile(arr2d, 1)
        p99 = np.percentile(arr2d, 99)

    if p99 <= p1:
        return np.zeros_like(arr2d, dtype=np.uint8)

    clipped = np.clip(arr2d, p1, p99)
    scaled = (clipped - p1) / (p99 - p1)
    return (scaled * 255).astype(np.uint8)


def to_uint8_mask(arr2d: np.ndarray) -> np.ndarray:
    max_val = arr2d.max() if arr2d.size else 0
    if max_val <= 1:
        out = (arr2d * 255).astype(np.uint8)
    else:
        out = np.clip(arr2d, 0, 255).astype(np.uint8)
    return out
...
def save_slices_png(
    ...
    for idx in slice_indices:
        ...
        mask_slice = _extract_slice(mask_img, plane, idx)
        ...
        Image.fromarray(mri_u8, mode="L").save(out_dir / f"{subject_id}_{plane}_{int(idx):03d}.png")
        Image.fromarray(mask_u8, mode="L").save(out_dir / f"{subject_id}_{plane}_{int(idx):03d}_mask.png")
...
def save_sample_png(
    ...
    Image.fromarray(mri_u8, mode="L").save(output_root / f"{subject_id}_{plane}_sample_mri.png")
    Image.fromarray(mask_u8, mode="L").save(output_root / f"{subject_id}_{plane}_sample_mask.png")
...
def process_subject(
    ...
    try:
        ...
        save_slices_png(mri_img, mask_img, "axial", axial_indices, subject_id, out_root)
        save_slices_png(mri_img, mask_img, "coronal", coronal_indices, subject_id, out_root)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract axial and coronal 2D PNG slices from synthstrip-data-v1-5-2d."
    )
    parser.add_argument("--data_root", required=True, help="Root directory containing subject folders.")
    parser.add_argument("--output_dir", required=True, help="Where to write axial, coronal, and sample PNG slices.")
...
def main():
    """
    Example:
        python scripts/extract_synthstrip_2d_png.py \\
            --data_root /data/synthstrip-data-v1-5-2d \\
            --output_dir /data/synthstrip-2d-slices \\
            --n_slices 50 --n_coronal 50 --resample_iso
    """
    args = parse_args()
