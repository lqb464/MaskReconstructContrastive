...
# Keep log-variance in a range that is stable under AMP (fp16 exp overflow).
_LOGVAR_MIN = -30.0
_LOGVAR_MAX = 20.0
...
def kl_divergence(mu: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
    ...
    # Compute in float32: under autocast, logvar.exp() easily overflows fp16.
    mu_f = mu.float().clamp(min=-1e4, max=1e4)
    logvar_f = logvar.float().clamp(_LOGVAR_MIN, _LOGVAR_MAX)
    kl = -0.5 * (1.0 + logvar_f - mu_f.pow(2) - logvar_f.exp())
    return kl.mean()


def reparameterize(mu: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
    # Sample in float32 so AMP fp16 does not overflow on exp / large std.
    dtype = mu.dtype
    logvar_f = logvar.float().clamp(_LOGVAR_MIN, _LOGVAR_MAX)
    mu_f = mu.float()
    std = torch.exp(0.5 * logvar_f)
    eps = torch.randn_like(std)
    return (mu_f + eps * std).to(dtype=dtype)
