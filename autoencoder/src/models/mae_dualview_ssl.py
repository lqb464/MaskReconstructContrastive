...
class MAEDualViewSSL(nn.Module):
    ...
    def output_state_dict_prefixes(self) -> tuple[str, ...]:
        return ("patch_decoder", "recon_head")

    def replace_output_channels(self, num_classes: int) -> None:
        n = int(num_classes)
        if n < 1:
            raise ValueError(f"num_classes must be >= 1, got {n}")
        in_ch = int(self.recon_head.in_channels)
        if int(self.recon_head.out_channels) == n:
            return
        self.recon_head = nn.Conv2d(in_ch, n, kernel_size=1)
        self.out_ch = n
