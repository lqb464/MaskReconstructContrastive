...
from ..downstream.model_utils import build_downstream_model
...
def build_skull_argparser() -> argparse.ArgumentParser:

    parser = build_argparser()

    grp = parser.add_argument_group("skull_stripping dataset")
    ...
    grp.add_argument("--val-every", type=int, default=1, help="Run validation every N epochs (default 1).")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = build_skull_argparser()
    args = parser.parse_args(argv)
    enforce_recon_only_args(args)
    return args

def make_dataloaders(
    train_ds: SkullStrippingDataset,
    cfg: ExperimentConfig,
    device: torch.device,
    val_ds: SkullStrippingDataset | None = None,
):
    ...
    seed_workers = bool(int(os.getenv("SKULL_STRIPPING_SEED_WORKERS", "0")))
...
def build_model(cfg: ExperimentConfig):
    if bool(getattr(cfg.training, "enable_contrastive", False)):
        raise ValueError("skull_stripping entrypoint forbids contrastive mode.")
    backbone = str(getattr(cfg.model, "backbone", "mae")).lower()
    if backbone not in {"mae", "vae"}:
        raise ValueError(f"skull_stripping supports backbone mae|vae, got {backbone!r}")
    return build_downstream_model(
        cfg,
        out_ch=1,
        enable_reconstruct=True,
        single_view=bool(cfg.training.single_view),
    )
...
def _validate_preprocess_meta(meta: dict, *, expected_image_size: int, data_dir: str | Path) -> None:
    ...
    if h != w:
        raise ValueError(
            ...
            "Current skull stripping model expects square --image-size."
        )
...
def run(args: argparse.Namespace) -> None:
    ...
    from .trainer import SkullStrippingTrainer
    ...
    print(f"[schedule] val_every={max(1, int(getattr(args, 'val_every', 1)))}")
    ...
    train_image_ext, train_skull_suffix = _resolve_dataset_file_pattern(
        train_meta,
    ...
    train_ds = SkullStrippingDataset(
        ...
        mask_suffix=train_skull_suffix,
    ...
    if bool(getattr(args, "dump_val_paths", False)) or bool(getattr(args, "dump_val_paths_only", False)):
        ...
        val_path_file = write_path_list(val_paths, out_dir / "val_paths_skull_stripping.txt")
        copied_n = copy_images_to_dir(val_paths, out_dir / "val_images_skull_stripping")
        print(f"[val_paths] task=skull_stripping count={len(val_paths)} file={val_path_file}")
        print(
            f"[val_paths] copied_images={copied_n} "
            f"dir={out_dir / 'val_images_skull_stripping'}"
        )
        for p in val_paths:
            print(p)
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    run_entrypoint(build_skull_argparser, run, argv=argv)
