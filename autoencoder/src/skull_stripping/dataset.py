...
_DEBUG_PAIR_ALIGNMENT_ENV = bool(int(os.getenv("SKULL_STRIPPING_DEBUG_PAIR_ALIGNMENT", "0")))
...
class SkullStrippingDataset(Dataset):
    ...
    Dataset for PNG -> skull stripping.
    ...
    def __init__(
        ...
        self.debug_pair_alignment_mod = max(1, int(os.getenv("SKULL_STRIPPING_DEBUG_PAIR_ALIGNMENT_MOD", "64")))
        ...
        if bool(debug_pair_alignment) and not _DEBUG_PAIR_ALIGNMENT_ENV:
            log.info("debug_pair_alignment requested but SKULL_STRIPPING_DEBUG_PAIR_ALIGNMENT=0, so debug is disabled.")
