...
class Trainer:
    def __init__(self, cfg: ExperimentConfig, device: torch.device):
        self.cfg = cfg
        self.device = device
        self.backbone = str(cfg.model.backbone).lower()
        ...
        if self.cfg.training.enable_contrastive:
            raise Exception("[Error] ver2 backbones support reconstruction-only training.")
    ...
    def _loss_pixel_mask(self, pixel_mask: torch.Tensor) -> torch.Tensor:
        if self._is_vae:
            return torch.zeros_like(pixel_mask)
        return pixel_mask
    ...
    def _visualize_recon(
        ...
        if self._is_vae:
            ...
            resid = (target_view - recon_img).abs().clamp(0, 1)
            out_path = str(self.vis_dir / f"{tag}_epoch_{epoch:03d}.png")
            save_image_grid(
                [shown_target, recon_img.clamp(0, 1), resid],
                [f"{tag}: target", "recon", "abs_resid"],
                out_path,
                panel_vmax={2: 0.05},
            )
            return
        ...
        resid = ((target_view - recon_img).abs() * pixel_mask).clamp(0, 1)
        ...
        out_path = str(self.vis_dir / f"{tag}_epoch_{epoch:03d}.png")
        save_image_grid(
            [target_view, pixel_mask, masked_in, recon_img.clamp(0, 1), resid],
            [f"{tag}: target", "mask", "masked_in", "recon", "abs_resid"],
            out_path,
            annotations={4: resid_ann},
            panel_vmax={4: 0.05},
        )

    def _write_vis_manifest(
        self,
        *,
        epoch: int,
        batch,
        ...
        if not getattr(self.cfg.logging, "vis_manifest", True):
            return
        paths: List[str] = []
        if isinstance(batch.get("path"), list):
            paths = [str(p) for p in batch["path"]]
        elif batch.get("path") is not None:
            paths = [str(batch["path"])]

        manifest = {
            "epoch": epoch,
            "backbone": self.backbone,
            "vis_batch_index": int(self.cfg.logging.vis_batch_index),
            "vis_mask_seed": int(mask_seed),
            "mask_seed_effective": int(mask_seed + epoch),
            "image_paths": paths,
            "mask_mean": float(pixel_mask.mean().item()),
            "mask_sum": float(pixel_mask.sum().item()),
        }
        out_path = self.vis_dir / f"vis_manifest_epoch_{epoch:03d}.json"
    ...
    def train_one_epoch(self, loader, epoch: int) -> Dict[str, float]:
        ...
        for batch in pbar:
            x, plane, pixel_mask = prepare_inputs(
                batch,
                device=self.device,
                cfg_mask=self.cfg.mask,
                backbone=self.backbone,
            )
            loss_mask = self._loss_pixel_mask(pixel_mask)
    ...
    def maybe_visualize(self, loader, epoch: int, tag: str):
        ...
        self.model.eval()
        batch = get_val_batch(loader, int(self.cfg.logging.vis_batch_index))
        vis_mask_seed = int(self.cfg.logging.vis_mask_seed) + int(epoch)
        ...
        x, plane, pixel_mask = prepare_inputs(
            ...
            cfg_mask=self.cfg.mask,
            backbone=self.backbone,
            mask_seed=vis_mask_seed if not self._is_vae else None,
        )

        self._write_vis_manifest(
            epoch=epoch,
            batch=batch,
            pixel_mask=pixel_mask,
            mask_seed=int(self.cfg.logging.vis_mask_seed),
        )
        ...
        if not self.cfg.training.single_view and recon_raw_flip is not None:
