...
def run_eval(
    *,
    ...
    for batch in loader:
        ...
        loss_mask = torch.zeros_like(pixel_mask) if is_vae else pixel_mask
...
def visualize_once(
    *,
    ...
    def _one_full(target_view: torch.Tensor, recon_img: torch.Tensor, suffix: str):
        resid = (target_view - recon_img).abs().clamp(0, 1)
        out_path = str(out_dir / f"{tag}{suffix}.png")
        save_image_grid(
            [target_view, recon_img.clamp(0, 1), resid],
            [f"{tag}{suffix}: target", "recon", "abs_resid"],
            out_path,
            panel_vmax={2: 0.05},
        )

    def _one_masked(target_view: torch.Tensor, recon_img: torch.Tensor, suffix: str):
        ...
        save_image_grid(
            ...
            [f"{tag}{suffix}: target", "mask", "masked_in", "recon", "abs_resid"],
            out_path,
            annotations={4: resid_ann},
            panel_vmax={4: 0.05},
        )

    draw = _one_full if is_vae else _one_masked
    draw(x, recon_img_orig, suffix="_orig")

    if not cfg.training.single_view and recon_img_flip is not None:
        x_flip = flip_lr(x)
        draw(x_flip, recon_img_flip, suffix="_flip")


def build_argparser():
    import argparse

    p = argparse.ArgumentParser("Standalone eval from MAE/VAE checkpoint (ver2)")
