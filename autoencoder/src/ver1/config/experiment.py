...
@dataclass
class ModelConfig:
    """Model architecture configuration (AE / MAE / VAE dual-view SSL)."""
    backbone: str = "ae"
    in_ch: int = 1
...
class ExperimentConfig:
    """Complete experiment configuration for AutoEncoder."""
    ...
    def from_args(cls, args: argparse.Namespace) -> "ExperimentConfig":
        backbone = str(getattr(args, "backbone", "ae")).lower()
        if getattr(args, "ae", False):
            backbone = "ae"
        elif getattr(args, "mae", False):
            backbone = "mae"
        ...
        cfg = cls(
            model=ModelConfig(
                backbone=backbone,
                in_ch=args.in_ch,
                base_ch=getattr(args, "base_ch", 16),
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser("AE / MAE / VAE Dual View SSL (brain MRI)")
    ...
    backbone.add_argument("--backbone", type=str, default="ae", choices=["ae", "mae", "vae"])
    backbone.add_argument("--ae", action="store_true", help="Use standard AutoEncoder backbone.")
    backbone.add_argument("--mae", action="store_true", help="Use Masked AutoEncoder backbone.")
    ...
    p.add_argument("--base-ch", type=int, default=16, help="Base channels for AE/VAE UNet backbone.")
    p.add_argument("--use-gn", action="store_true", help="Use GroupNorm in conv UNet blocks.")
    p.add_argument("--use-se", action="store_true", help="Use SE blocks in deepest UNet encoder stages (AE/VAE).")
    ...
    p.add_argument("--out-dir", type=str, default="runs_ssl_ae")
