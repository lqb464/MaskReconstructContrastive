...
def plot_training_curves(log_csv_path: Path, output_dir: Path):
    ...
    plt.figure(figsize=(6, 4))
    plt.plot(df['epoch'], df['train_recon_masked'], label='train_skulled')
...
def plot_loss_decomposition_curves(loss_decomp_csv_path: Path, output_dir: Path):
    ...
    1. Reconstruction loss (orig / flip / total) for train and val in one figure
    2. Contrastive loss for train and val in one figure
    3. Total loss (recon + contrast) for train and val in one figure
    """
