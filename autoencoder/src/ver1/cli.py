...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m autoencoder.src.ver1.cli",
        description="AE / MAE / VAE dual-view SSL training for brain MRI.",
    )
    parser.add_argument(
        "command",
        nargs="?",
...
def main(argv: Optional[Sequence[str]] = None) -> object:
    ...
    if not raw_argv or raw_argv[0] in {"-h", "--help"}:
        ...
        print("  eval  -> eval-ssl")
        print("\nBackbones:")
        print("  --ae   standard AutoEncoder")
        print("  --mae  Masked AutoEncoder")
