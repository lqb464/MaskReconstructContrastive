...
from ..downstream.dual_view import dual_view_loss_and_logits
from ..downstream.model_utils import forward_dual_recon
...
class TissueSegmentationTrainer:
    ...
    def _assert_model_contract(self) -> None:
        ...
        if hasattr(self.model, "recon_head"):
            out_ch = int(self.model.recon_head.out_channels)
            if out_ch != int(self.num_classes):
                raise RuntimeError(
                    f"recon_head output channels ({out_ch}) != num_classes ({self.num_classes})"
                )
        elif hasattr(self.model, "decoder") and hasattr(self.model.decoder, "out_conv"):
            out_ch = int(self.model.decoder.out_conv.out_channels)
            if out_ch != int(self.num_classes):
                raise RuntimeError(
                    f"decoder.out_conv output channels ({out_ch}) != num_classes ({self.num_classes})"
                )
        else:
            raise RuntimeError("Model must expose recon_head (MAE) or decoder.out_conv (VAE) for tissue segmentation.")
    ...
    # ------------------------------------------------------------------
    # Extension hooks (no-op in base; override in subclasses)
    # ------------------------------------------------------------------

    def _on_epoch_start(self, training: bool) -> None:
        """Called at the beginning of each _run_epoch pass."""
    ...
    def _on_batch_end_with_preds(
        ...
        """Called after argmax + ignore-index masking, before the next batch."""

    def _on_epoch_end_stats(self, stats: dict) -> dict:
        """Called with the final stats dict before returning from _run_epoch."""
        return stats
    ...
    def _post_epoch_hook(
        ...
        *,
        ...
        """Called at the end of each epoch in fit(), after all standard logging."""
    ...
    def _forward_views(
        ...
        recon1, recon2 = forward_dual_recon(self.model, x, plane_one_hot, pixel_mask)
        ...
        return dual_view_loss_and_logits(recon1, recon2, target, self.criterion)
        ...
        *,
        ...
        for batch_idx, batch in enumerate(progress):
            ...
            with torch.no_grad():
                ...
                self._on_batch_end_with_preds(pred, y, valid)
        ...
        return self._on_epoch_end_stats(stats)
