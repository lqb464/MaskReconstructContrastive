...
```bash
python -m autoencoder.src.cli <command> [args...]
```
...
```bash
python -m autoencoder.src.cli train-ssl \
...
```bash
python -m autoencoder.src.cli train-tissue \
  --mae \
  --image-size 256 \
...
```bash
python -m autoencoder.src.cli train-skull \
  --vae \
