...
def finalize_iou(
    ...
    *,
    ...
    """
    Convert buffers to per-class IoU and validity mask.

    IoU_c = I_c / (|pred_c| + |tgt_c| - I_c) = I_c / (denom_c - I_c)
    """
    _validate_empty_handling(empty_handling)
    ...
    iou = torch.zeros_like(denom, dtype=torch.float64)
    valid = denom > 0

    if valid.any():
        union = denom[valid] - inter[valid]
        iou[valid] = inter[valid] / (union + float(eps))
