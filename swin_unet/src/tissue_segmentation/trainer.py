...
class TissueSegmentationTrainer:
    ...
    def _forward_views(
        ...
        if recon1.ndim != 4 or int(recon1.shape[1]) != self.num_classes:
            raise RuntimeError(
                "Unexpected reconstruction head shape: "
                f"recon1={tuple(recon1.shape)} (expected [B,{self.num_classes},H,W])."
            )

        single_view = bool(getattr(self.cfg.training, "single_view", False)) or bool(
            getattr(self.model, "single_view", False)
        )
        if single_view or recon2 is None:
            if not single_view and recon2 is None:
                raise RuntimeError(
                    "Dual-view mode requires recon2, but model returned recon2=None. "
                    "Pass --single-view or build the model with single_view=False."
                )
    ...
    # ------------------------------------------------------------------
    # Extension hooks (no-op in base; override in subclasses e.g. tumor)
    # ------------------------------------------------------------------
