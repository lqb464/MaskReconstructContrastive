...
def finalize_region_iou(
    buffers: RegionDiceBuffers,
    *,
    eps: float = 1e-6,
) -> Dict[str, float]:
    """
    Compute final IoU scores for each region from accumulated buffers.

    Returns:
        dict mapping region name (e.g. 'wt', 'tc', 'et') -> IoU value in [0, 1].
    """
    results: Dict[str, float] = {}
    for name in buffers.intersection:
        inter = float(buffers.intersection[name].item())
        ...
        if union < eps:
            ...
            results[name] = inter / (union + eps)
    return results


def compute_hd95_2d(pred_mask: np.ndarray, tgt_mask: np.ndarray) -> float:
    """
    Compute 95% Hausdorff Distance between two binary 2D masks in pixels.
    """
    ...
    coords_pred = get_boundary_coords(pred_mask)
    coords_tgt = get_boundary_coords(tgt_mask)

    if coords_pred is None and coords_tgt is None:
        return 0.0  # both empty, distance is 0
    if coords_pred is None or coords_tgt is None:
        return float("nan")  # one is empty, distance is undefined/nan

    d_pred_to_tgt = cdist(coords_pred, coords_tgt).min(axis=1)
    d_tgt_to_pred = cdist(coords_tgt, coords_pred).min(axis=1)

    return float(max(np.percentile(d_pred_to_tgt, 95), np.percentile(d_tgt_to_pred, 95)))
